...
if [ -n "$str" ]; then
  if [ $JavaVersion -ge 11 ] ; then
    # For G1
    JAVA_OPTS="-server -Xms256m -Xmx512m -XX:+UseG1GC -XX:MaxGCPauseMillis=250 -XX:+UseGCOverheadLimit -XX:+ExplicitGCInvokesConcurrent $JAVA_OPTS"
  else
	  JAVA_OPTS="-server -Xms256m -Xmx512m -Xmn128m -XX:SurvivorRatio=2 -XX:PermSize=96m -XX:MaxPermSize=256m -XX:MaxTenuringThreshold=15 -XX:+DisableExplicitGC $JAVA_OPTS"
	fi
else
	JAVA_OPTS="-server -Xms256m -Xmx512m -XX:NewSize=256m -XX:MaxNewSize=256m -XX:MaxPermSize=128m $JAVA_OPTS"
fi
...
cd $bin_abs_path
  ...
  echo "********************************************************"
  echo "Waiting for the server to start on ${WAIT_SERVICES}"
  echo "********************************************************"
  ./wait-for-it.sh -t 0 ${WAIT_SERVICES} -- echo "*******  ${WAIT_SERVICES} Server has started"
  echo "${WAIT_SERVICES} SERVER IS UP"
